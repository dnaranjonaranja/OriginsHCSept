// brand.jsx — Sistema visual de 8D
...
// ── MARK PRINCIPAL ──────────────────────────────────────────────────────────
// El asterisco 8D — los 4 trazos juntos. Opcionalmente con fondo navy.
...
// ── LOCKUP DE MARCA (logo + nombre 8D — versión horizontal grande) ────────
// Lockup como aparece en el branding: asterisco + "8D" en serif al lado.
function HCLockup({ size = 44 }) {
  return (
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} style={{ display: 'block' }}>
      ...
      {[0.30, 0.55, 0.80, 1.0].map((rr, i) =>
        <circle key={i} cx={half} cy={half} r={r*rr} fill="none" stroke="var(--line-soft)" strokeWidth="0.5" />
      )}
      {angles.map((a, i) =>
        <line key={i}
          x1={half} y1={half}
      ...
      {pillars.map((p, i) => {
        const lx = half + Math.cos(angles[i]) * (r + 28);
        const ly = half + Math.sin(angles[i]) * (r + 28);
        return (
          <text key={p} x={lx} y={ly}
            textAnchor="middle" dominantBaseline="middle"
            fill={colors[i]} fontSize="10" fontWeight="600"
